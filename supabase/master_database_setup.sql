-- ========================================================================================
-- TIME2TRADE CRM - CONSOLIDATED MASTER DATABASE SCRIPT (ALL-IN-ONE)
-- Safe, Idempotent, and Non-Destructive: Preserves all existing real data.
-- Run this in your Supabase SQL Editor (Dashboard -> SQL Editor -> New Query -> Run)
-- ========================================================================================

-- ====================================================================
-- SECTION 1: EXTENSIONS & POSTGRES CONFIGURATION
-- ====================================================================
...
-- Ensure all required columns exist in users table if table was created earlier
...
-- Update role check constraint to allow 'admin', 'employee', and 'pending'
...
ALTER TABLE public.users ADD CONSTRAINT users_approval_status_check CHECK (approval_status IN ('pending_admin_review', 'approved', 'rejected'));
...
-- 5. PAYMENTS TABLE (SUPPORTS PROSPECTS & DIRECT CLIENTS)
...
-- Ensure payments columns exist and trader_id is nullable (allows direct payment submissions)
...
-- Trigger Function: Recalculate Trader Metrics
...
-- Trigger Function: Recalculate Trader Profit Shared
...
-- Prevents 401 Unauthorized / 403 Forbidden errors across all endpoints
-- ====================================================================
...
-- ====================================================================
-- SECTION 7: CLEANUP OF OLD MOCK DATA (PRESERVES REAL ACCOUNTS)
-- ====================================================================
-- Removes hardcoded sample mock records while strictly preserving your real team:
-- (e.g. Karthik Muni, Madhan, Arthi, Dhanalakshmi, Varsha sree, Anupriya, Sravanthi, Lokesh, etc.)
...
-- 7. Remove mock demo employees (only capitalgrow.com & mock names)
...
-- 8. Ensure Admin account exists and is approved
...
-- ====================================================================
-- SECTION 8: RELOAD POSTGREST CACHE (IMMEDIATE DEPLOYMENT)
-- ====================================================================
NOTIFY pgrst, 'reload schema';

-- Output confirmation
SELECT 'Time2Trade CRM database setup completed successfully! All tables, columns, indexes, RLS policies, storage buckets, and triggers are active.' AS status;
