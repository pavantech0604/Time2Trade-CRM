...
CREATE TABLE IF NOT EXISTS public.leads (
    ...
    assigned_to UUID REFERENCES public.users(id) ON DELETE SET NULL, -- Telecaller
    rm_assigned_to UUID REFERENCES public.users(id) ON DELETE SET NULL, -- Relationship Manager
...
CREATE TABLE IF NOT EXISTS public.active_traders (
    ...
    name TEXT NOT NULL,
    phone TEXT NOT NULL,
    rm_assigned_to UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
...
CREATE POLICY "Telecaller lead access" ON public.leads FOR SELECT USING (assigned_to = auth.uid());
CREATE POLICY "Telecaller lead update" ON public.leads FOR UPDATE USING (assigned_to = auth.uid());
CREATE POLICY "RM lead access" ON public.leads FOR SELECT USING (rm_assigned_to = auth.uid() OR assigned_to = auth.uid());
CREATE POLICY "RM lead update" ON public.leads FOR UPDATE USING (rm_assigned_to = auth.uid());
...
CREATE POLICY "RM trader access" ON public.active_traders FOR SELECT USING (rm_assigned_to = auth.uid());
CREATE POLICY "RM trader modify" ON public.active_traders FOR ALL USING (rm_assigned_to = auth.uid());

-- Trading Days Policy
CREATE POLICY "Admin trading days access" ON public.trading_days FOR ALL USING (public.get_current_role() = 'admin');
CREATE POLICY "RM trading days access" ON public.trading_days FOR ALL USING (
    EXISTS (
...
CREATE POLICY "RM payments access" ON public.payments FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM public.active_traders
